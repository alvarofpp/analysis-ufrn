...
class TaxaDeAprovacaoPage(PageView):
    ...
    def intro(self):
        ...
        st.markdown('''
        Aqui você conseguirá ver a taxa de aprovação das disciplinas. Foram disponibilizados dois tipos de visualizações: por docente e por componente curricular.
        
        Para a realização deste trabalho foram utilizados 4 conjuntos de dados presentes na base de [Dados abertos da UFRN](http://dados.ufrn.br/):
        ...
        Foram usados apenas dados de turmas consolidadas e as matrículas com status de aprovado ou reprovado.
    ...
    def section_01(self):
        ...
        taxa_aprovacao_docente = self.data[self.data['siape'] == option_docente][['nome_componente', 'taxa_aprovacao',]]
        taxa_aprovacao_docente = taxa_aprovacao_docente.sort_values('taxa_aprovacao', ascending=False).reset_index(drop=True)
    ...
    def section_02(self):
        ...
        Aqui você poderá buscar pelo componente curricular e visualizar a taxa de aprovação por docente que já lecionou esse componente.
        ''', unsafe_allow_html=True)
        option_docente = st.selectbox('Componente Curricular',
                                      options=self.data['id_componente_curricular'].unique().tolist(),
                                      format_func=self.format_func_componentes_curriculares),

        taxa_aprovacao_componente = self.data[self.data['id_componente_curricular'] == option_docente][['nome_docente', 'taxa_aprovacao',]]
        taxa_aprovacao_componente = taxa_aprovacao_componente.sort_values('taxa_aprovacao', ascending=False).reset_index(drop=True)
